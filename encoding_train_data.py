...
data_processed_train = pd.read_csv(r'X:\Hackathon\AV - AMEXPERT\train_amex\data_processed_train2.csv')
...
df = data_processed_train[keep_columns]
...
df_X = df.drop(['is_click'], axis = 1)
df_y = pd.DataFrame(df['is_click'])
df_train_X, df_va_X, df_train_y, df_va_y = train_test_split(df_X, df_y, test_size = 0.2, random_state=0)
...
np.save(file = r'X:\Hackathon\AV - AMEXPERT\train_amex\numpy_inputs\np_train_X', arr=label_encode_it(df_train_X))
np.save(file = r'X:\Hackathon\AV - AMEXPERT\train_amex\numpy_inputs\np_va_X', arr=label_encode_it(df_va_X))
np.save(file = r'X:\Hackathon\AV - AMEXPERT\train_amex\numpy_inputs\np_train_y', arr=label_encode_it(df_train_y))
np.save(file = r'X:\Hackathon\AV - AMEXPERT\train_amex\numpy_inputs\np_va_y', arr=label_encode_it(df_va_y))
...
##### Conversion to libffm format###
##### xlearn has internal numpy to libffm conversion format #####
...
convert_to_ffm(df_train, type='train', numerics=['total_visit'],
...
convert_to_ffm(df_va, type='va', numerics=['total_visit'],
               categories=['product', 'campaign_id', 'webpage_id', 'product_category_1',
                           'user_group_id', 'gender', 'age_level', 'user_depth', 'var_1', 'is_click', 'view_cat', 'interest_cat'],
               features=['product', 'campaign_id', 'webpage_id', 'product_category_1',
                         'user_group_id', 'gender', 'age_level', 'user_depth', 'var_1', 'total_visit', 'view_cat', 'interest_cat'])
...
#data_ffm = open(r'X:\Hackathon\AV - AMEXPERT\train_amex\data_ffm.txt', "r")
